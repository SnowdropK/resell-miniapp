  ...
  <view class="orderItemBox" @click="goToOrderDetail">
    ...
    >
      ...
      <view class="infoBox">
        ...
        <view class="headBox">
          ...
          <view>
            <view class="priceTag">
              ￥<text class="priceNum">{{ toYuan(item.goodsPrice) }}</text>
            </view>
            <view class="num">x {{ item.goodsNum || 0 }}</view>
          </view>
        </view>
      </view>
    </view>
    ...
    <view class="btBox">
      ...
      <text v-if="true" class="actualPrice">
        ...
        <text class="priceNum">{{ toYuan(info.orderAmount) }}</text>
      </text>
    </view>
    <!-- 操作按钮 -->
    <view
...
export default {
  ...
  methods: {
    ...
    goToOrderDetail() {
      ...
      const { id } = this.info;
      uni.navigateTo({
        url: `/pages/me/myOrder/buyOrderDetail/index?id=${id}&orderType=${orderType}`,
      });
    },
    async operateOrder() {
